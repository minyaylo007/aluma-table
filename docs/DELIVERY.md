...
- **Як зараз** — сервіс на машині працює з клону `/opt/ihor/aluma-table` на коміті `563edc8` (detached HEAD) з
  юнітами, поставленими руками; health — `0.4.0-box` без `git_sha`. Репозиторій **публічний з 2026-09-12**, `master`
  захищений (у нього потрапляє лише PR із п'ятьма зеленими перевірками), окруження `production` створене, вимикач
  випуску вимкнений — конвеєр перевіряє, але нічого не випускає й не викочує.

- **Після установки агента (T040)** — версії з GitHub ставить агент: каталоги `releases/vX.Y.Z` + `current`, юніти з
  `current/`, health показує номер і sha версії. Це настає після «так» власника на першу установку (T040).

**Публікація (12.09.2026).** Публічним став **новий** репозиторій
[`minyaylo007/aluma-table`](https://github.com/minyaylo007/aluma-table) з чистою історією: знімок `master` одним
комітом. Стара історія (114 комітів, усі старі sha) лишилась у приватному архіві
`minyaylo007/aluma-table-archive-2026-09` — у ній описи слабких місць машини, а прибрати їх з історії можна лише
переписуванням, яке заборонене (конституція VI). Ім'я репозиторію не змінилось, тож `origin` і посилання виду
`https://github.com/minyaylo007/aluma-table/releases/…` лишились ті самі; посилання на прогони Actions ДО 12.09
ведуть на архівний репозиторій. Подробиці, скани й живі перевірки —
[verification.md](../specs/001-release-pipeline/verification.md), розділ Phase 3.

Позначка **«не перевірено, після Phase 3»** — твердження, яке ще не звірене з живою системою; після 12.09 таких
лишилось мало, і всі вони чекають агента (T040). Усі — і в розділі «Розрив зі стандартом флоту».
...
| окруження `production` у GitHub | **створене 12.09**: політика веток — рівно одна, `master`; змінна `ALUMA_HEALTH_URL` = `https://table.central-aparts.store/api/fx/health` (адреса жива: health віддає `0.4.0-box`) | те саме; додасться активна версія після першого деплою |
...
- **Діє з 12.09.2026** (перевірено вживу, `verification.md` розділ Phase 3): ruleset `master` — лише через PR,
  обов'язкові перевірки `lint`, `test (3.12)`, `test (3.13)`, `build`, `browser`, 0 схвалень, **без обходу** (і для
  власника), заборона force-push і видалення; метод злиття — squash (інші два вимкнені в налаштуваннях репозиторію),
  заголовок PR = повідомлення коміту в стилі проєкту `type(scope): опис`; авто-merge після зелених перевірок; гілка
  після злиття GitHub прибирає сам. **Шлях усіх сесій: гілка → PR → авто-merge.** Перемотка в `master` більше не
  працює — сервер відхиляє (`GH013: Changes must be made through a pull request`).
- **Тонкість, яку легко втратити:** у правилі `pull_request` параметр
  `require_extra_approval_for_unattributed_changes` задано **явно `false`**. GitHub підставляє `true`, якщо його не
  написати, і тоді PR із комітом, чий автор не прив'язаний до акаунта GitHub (у нас це трейлер `Co-Authored-By`
  помічника), висить в очікуванні схвалення — авто-merge не спрацьовує, хоча схвалень потрібно нуль. Значення
  закріплене тестом `tests/test_rulesets.py`; правити файл правила, а не налаштування в інтерфейсі.
...
підтверджено прогоном [34608006392](https://github.com/minyaylo007/aluma-table-archive-2026-09/actions/runs/34608006392).
...
- **Вимикач випуску `RELEASES_ENABLED`** (змінна репозиторію): `release` і за ним `deploy` виконуються лише при
  ...
  [34614386303](https://github.com/minyaylo007/aluma-table-archive-2026-09/actions/runs/34614386303),
  [34616141687](https://github.com/minyaylo007/aluma-table-archive-2026-09/actions/runs/34616141687)); тегів і релізів — 0.
...
- Незмінність (діє з 12.09, перевірено вживу): ruleset `tags-v-lock` — тег `v*` **не перезапише й не видалить ніхто**,
  включно з конвеєром (`Cannot update this protected ref`, `Cannot delete this tag` — обидва 422 на пробному тезі).
  Створення тега НЕ обмежене: обходу для застосунку Actions GitHub не дає (розділ 9, п.1), а з обмеженим `creation`
  конвеєр не поставив би тег узагалі. Налаштування «незмінні релізи» на цьому акаунті недоступне — поля
  `immutable_releases` API не має; його роль і виконує `tags-v-lock`.
...
- Змінні GitHub (не секрети): `ALUMA_HEALTH_URL` (окруження `production`) — **створена 12.09**;
  `RELEASES_ENABLED` (репозиторій) — **як зараз не створена** (вимикач випуску, розділ 4).
- Сканування секретів GitHub і захист від їх push — **увімкнені 12.09** (доступні лише на публічному репозиторії);
  попереджень 0. Push protection вмикається окремим викликом: разом із самим скануванням в одному `PATCH` воно
  лишається `disabled`.
...
- Перед публікацією перевірено всю історію старого репозиторію — [secret-scan-report.md](../specs/001-release-pipeline/secret-scan-report.md)
  ([`scripts/secret_scan.py`](../scripts/secret_scan.py) + gitleaks) — і окремо дерево, яке пішло в публічний
  репозиторій (`verification.md`, Phase 3). Значень секретів не друкувати ніде.
...
| GitHub | `minyaylo007/aluma-table` — **публічний з 12.09.2026**, історія чиста (знімок одним комітом); стара історія і всі старі sha — у приватному архіві `minyaylo007/aluma-table-archive-2026-09` (Actions там вимкнені). Правила: `master`, `tags-v-lock`, `production-lock` — усі `active`, обходу немає ні в одному |
...
1. **Обхід для застосунку GitHub Actions у правилах** — GitHub його не дає в особистому репозиторії:
   `Actor GitHub Actions integration must be part of the ruleset source or owner organization` (422). Тому
   застосовано три правила **без обходу** (запасний варіант research.md R4): `master` (PR + 5 перевірок, squash, без
   обходу), `tags-v-lock` (`v*` — оновлення й видалення заборонені всім, включно з конвеєром), `production-lock`
   (видалення заборонено всім). **Слабше за задум:** людина може вручну створити тег `v*` або зсунути
   `refs/heads/production`. **Компенсація:** агент ставить лише коміт опублікованого конвеєром тега з повним набором
   файлів релізу і збігом сум ([agent-cli.md](../specs/001-release-pipeline/contracts/agent-cli.md),
   [production-ref.md](../specs/001-release-pipeline/contracts/production-ref.md)), тому вручну можна викотити тільки
   вже випущену версію. **Стандарт флоту:** обхід для Actions потребує організації GitHub — рішення власника, якщо
   колись знадобиться. Решта пункту закрита 12.09: захист `master`, окруження `production` і сканування секретів
   застосовані й перевірені вживу.
...
6. **Виняток для GitHub Actions** (research.md R4) — перевірений 12.09 і **не працює** на особистому репозиторії;
   діє запасний варіант, див. п.1 вище. Цей пункт закритий: невизначеності більше немає.
