...
- **Як зараз** — сервіс на машині працює з клону `/opt/ihor/aluma-table` на коміті `563edc8` (detached HEAD) з
  ...
  захищений (у нього потрапляє лише PR із шістьма зеленими перевірками), окруження `production` створене, вимикач
...
- **Діє з 12.09.2026** (перевірено вживу, `verification.md` розділ Phase 3): ruleset `master` — лише через PR,
  обов'язкові перевірки `secret-scan`, `lint`, `test (3.12)`, `test (3.13)`, `build`, `browser`, 0 схвалень,
  **без обходу** (і для
...
|---|---|---|
| `secret-scan` | [`scripts/secret_scan.py`](../scripts/secret_scan.py) по робочому дереву; нові місця проти [базової лінії](../.github/secret-scan-baseline.json) роблять перевірку червоною | — (лише stdlib) |
...
**`secret-scan` іде завжди** — без умов і без фільтрів шляхів, зокрема й на коміті в один `.md`: правило флоту
13.09.2026. Саме такий скан знайшов в іншому проєкті коди від дверей і пароль WiFi — **у документах**, не в коді.
Сторонніх сканерів не ставимо (ні `gitleaks`, ні `trufflehog`): свій, на самій stdlib, секунди роботи й нуль
оплачених хвилин (репозиторій публічний). Значення не друкуються ніде: у журнал іде лише підсумок за правилами,
звіт зі шляхами лишається в `$RUNNER_TEMP` прогону (правило власника 10). Базова лінія — перелік уже переглянутих
місць виду «правило, шлях, скільки», без значень; її оновлюють свідомо
(`--baseline … --update-baseline`), і найгірші класи (ключі, приватні ключі, файли `.env`, бази) до неї не
потрапляють ніколи — це закріплено `tests/test_secret_scan.py`. Разовий скан усієї історії, публічної й
архівної, — [`security/secret-scan-2026-09-13.md`](security/secret-scan-2026-09-13.md).
